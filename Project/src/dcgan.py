...
class DCGAN(object):
    ...
    def train(self, epochs, batch_size, X_train, save_result_dir, save_weight_dir):
        g_losses = []
        d_losses = []
        # noiseの発生を一定にする。
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 50 == 0:
                self.g.save_weights(os.path.join(save_weight_dir, "generator_" + str(epoch) + '.h5'), True)
                self.d.save_weights(os.path.join(save_weight_dir, "discriminator_" + str(epoch) + '.h5'), True)
